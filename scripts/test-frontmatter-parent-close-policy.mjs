...
writeAgent(
	...
	["Reply with exactly `FM_CONTINUE_OK`."].join("\n"),
);
...
writeAgent(
	...
	["Reply with exactly `FM_TERMINATE_OK`."].join("\n"),
);
...
writeAgent(
	...
	["Reply with exactly `FM_DEFAULT_OK`."].join("\n"),
);
...
try {
	...
	for (const [label, d] of [
		...
		if (metadata.data.parentClosePolicy !== expectedPolicy) {
			throw new Error(
				`${label} child expected parentClosePolicy "${expectedPolicy}", got "${metadata.data.parentClosePolicy}"`,
			);
		}

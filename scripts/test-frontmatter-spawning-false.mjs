	...
	try {
		...
		if (subagentCheck) {
			if (subagentCheck.details?.available !== false) {
				throw new Error(
					"subagent tool should be denied for spawning:false, but check_tool_available reports it as available.",
				);
			}
			foundDenied = true;
			...
			try {
				...
				for (const file of snapshotFiles) {
					...
					if (
						...
						!active.includes("subagent_resume") &&
						!active.includes("subagents_list")
					) {
						foundDenied = true;
						break;
					}
		...
		if (!foundDenied) {
			throw new Error(
				"Could not verify subagent tools are denied. No check_tool_available result or snapshot evidence found.",
			);
		}

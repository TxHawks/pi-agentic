	...
	while (Date.now() < deadline) {
		...
		if (parent) {
			...
			if (status === "failed" || status === "cancelled") {
				throw new Error(
					`Parent-visible subagent result was ${status}: ${JSON.stringify(toolResult?.details, null, 2)}`,
				);
			}
		...
		? JSON.stringify(
				{
					...
					assistants: getAssistantTexts(lastParent.events),
					toolResults: getSubagentToolResults(lastParent.events).map((result) => result.details),
				},
				null,
				2,
			)
...
async function waitForChildSurfaceCleanup(scenario, childPaneId) {
	...
	throw new Error(
		`Herdr child surface ${scenario.childName} (${childPaneId}) was still present after child completion`,
	);
}
...
function validateParentOutcome(scenario, toolResult) {
	...
	if (details.status !== "completed")
		throw new Error(`Expected completed child status, got ${details.status ?? "missing"}`);
	if (details.mode !== scenario.childMode)
		throw new Error(`Expected ${scenario.childMode} child mode, got ${details.mode ?? "missing"}`);
	if (details.deliveryState !== "awaited")
		throw new Error(`Expected awaited delivery, got ${details.deliveryState ?? "missing"}`);
	if (details.async !== false)
		throw new Error(`Expected blocking child async=false, got ${details.async ?? "missing"}`);
	...
	if (!details.sessionFile || !existsSync(details.sessionFile))
		throw new Error("Parent-visible child result missing existing sessionFile");
...
async function runScenario(ctx, scenario, liveModel) {
	...
	mkdirSync(join(ctx.workDir, scenario.childWorkspaceName), {
		recursive: true,
	});
	...
	try {
		...
		if (!scenario.expectChildPane && observedChildPaneId) {
			throw new Error(
				`Background scenario ${scenario.name} unexpectedly opened Herdr child pane ${observedChildPaneId}`,
			);
		}

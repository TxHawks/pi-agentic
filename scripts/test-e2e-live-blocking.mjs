...
try {
	while (Date.now() < deadline) {
		...
		const textOnly = assistantMessages.filter((m) => {
		...
		if (textOnly.length > 0) {
			throw new Error(
				`Parent did extra assistant work during the blocking turn: ${textOnly.map((m) => JSON.stringify(m.message?.content)).join(", ")}`,
			);
		}

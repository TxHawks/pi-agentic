...
const PARENT_MODEL = process.env.FORK_LIVE_PARENT_MODEL ?? "nahcrof/deepseek-v4-flash:medium";
...
if (!childSession) {
	...
	if (maxUsage < 150_000) {
		console.log(
			`  WARN max usage ${maxUsage} stayed below 150K; the test may not have actually exercised the threshold`,
		);
	}
}

...
describe("result router", () => {
	...
	it("tells the parent an early stop was instructed and not to resume it", () => {
		...
		const message = sent[0]?.message as { content?: string } | undefined;
	...
	it("routes detached completion through one parent-visible result", () => {
		...
		const message = sent[0].message as {
			customType: string;
			details: { id: string; deliveryState: string; status: string };
		};
		assert.equal(message.customType, "subagent_result");
		assert.equal(message.details.id, running.id);
		assert.equal(message.details.deliveryState, "detached");
	...
	it("appends final child context usage after the session reference", () => {
		...
		const message = sent[0].message as {
			content: string;
			details: { contextTokens: number; contextWindow: number };
		};
		assert.match(
			message.content,
		...
		assert.equal(message.details.contextTokens, 145_000);
		assert.equal(message.details.contextWindow, 200_000);
	});
	...
	it("keeps final context telemetry structured when the agent definition hides it from the parent message", () => {
		...
		assert.doesNotMatch(message.content, /Sub-agent context:/);
		assert.equal(message.details.contextTokens, 145_000);
		assert.equal(message.details.contextWindow, 200_000);
	});
	...
	it("delivers salvaged child output with provider errors", () => {
		...
		const message = sent[0].message as { content: string };
		assert.match(message.content, /Last output before the failure/);
		assert.match(message.content, /Completed the requested implementation\./);
		assert.doesNotMatch(message.content, /did not produce a result/);
		assert.match(message.content, /Sub-agent context: 145K\/200K tokens \(72%\) used at finish\.$/);
	});
	...
	it("does not present runtime diagnostics as salvaged child output", () => {
		...
		const message = sent[0].message as { content: string };
		assert.match(message.content, /did not produce a result/);
		assert.doesNotMatch(message.content, /Last output before the failure/);
		assert.doesNotMatch(message.content, /provider stack trace/);
	});
	...
	it("routes child pings without caching a completed result", () => {
		...
		const message = sent[0].message as {
			customType: string;
			content: string;
			details: { id: string; message: string };
		};
		assert.equal(message.customType, "subagent_ping");
		assert.equal(message.details.id, running.id);
		assert.equal(message.details.message, "Need parent input.");
		assert.match(message.content, /Need parent input\./);
		assert.deepEqual(sent[0].options, {
			triggerTurn: true,
			deliverAs: "steer",
		});
